...
export const Login = () => {
  ...
  return (
    ...
    >
      ...
      {/* Box de Login */}
      <div className="w-full max-w-md bg-[var(--bg-card)] rounded-[2.5rem] md:rounded-[3rem] shadow-2xl p-6 md:p-10 border border-[var(--border-color)] relative overflow-hidden flex flex-col items-center z-10">
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          ...
          <div className="mb-6 md:mb-8 flex justify-center w-full">
            {settings?.logoLoginTopUrl ? (
              ...
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl bg-primary flex items-center justify-center text-[var(--text-inverse)] shadow-xl shadow-primary/20">
          ...
          <div className="text-center mb-6 md:mb-8">
            <h1 className="text-xl md:text-3xl font-black text-[var(--text-main)] tracking-tight leading-tight">
              {settings?.appName || 'Meu Cronograma'}
            </h1>
            <div className="flex items-center justify-center gap-2 mt-2">
              <div className="h-[1px] w-4 bg-primary/30 rounded-full" />
              <p className="text-[var(--text-muted)] text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em]">Acesso Restrito</p>
              <div className="h-[1px] w-4 bg-primary/30 rounded-full" />
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 w-full">
            <div className="space-y-2">
              <label className="block text-[9px] md:text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest text-center">Credenciais de Acesso</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <LockIcon className="w-4 h-4 text-[var(--text-muted)] group-focus-within:text-primary transition-colors" />
                </div>
                <input
                  type="password"
                  required
            ...
            {error && (
              <div className="bg-red-500/10 border-2 border-red-500/20 text-red-500 text-[10px] md:text-xs font-black p-3 rounded-xl text-center animate-shake flex items-center justify-center gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary text-[var(--text-inverse)] flex items-center justify-center gap-2 py-3.5 md:py-4 rounded-xl text-sm md:text-base font-black uppercase tracking-widest shadow-xl shadow-primary/20 active:scale-[0.97] hover:brightness-110 transition-all disabled:opacity-50"
        ...
        <div className="w-full flex flex-col items-center gap-4 md:gap-6 mt-6 md:mt-8 pt-6 md:pt-8 border-t-2 border-[var(--border-color)]/50">
          {settings?.instagramUrl && (
            <a
              href={settings.instagramUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center justify-center gap-2 px-6 py-3 bg-[var(--bg-input)] text-[var(--text-main)] rounded-xl border-2 border-[var(--border-color)] hover:bg-primary hover:text-[var(--text-inverse)] hover:border-primary transition-all duration-500 w-full shadow-sm hover:shadow-lg hover:shadow-primary/10"
            >
              <Instagram className="w-4 h-4 group-hover:scale-110 transition-transform" />
              <span className="font-black text-[10px] md:text-xs uppercase tracking-widest">Nossas Redes</span>
            </a>
          )}
